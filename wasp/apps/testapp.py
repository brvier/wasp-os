...
class TestApp:
        ...
        if self.test == "Checkbox":
            ...
            if self._spinner.touch(event):
                ...
                if len(notifications) > self._spinner.value:
                    wasp.system.unnotify(next(iter(notifications.keys())))
                else:
                    wasp.system.notify(wasp.watch.rtc.get_uptime_ms(), {"src": "Hangouts", "title": "A Name really long title", "body": "message contents"})
        elif self.test == "RLE":
            self._benchmark_rle()
        elif self.test == "String":
            self._benchmark_string()
        elif self.test == "Touch":
            wasp.watch.drawable.string("({}, {})".format(event[1], event[2]), 0, 108, width=240)
        elif self.test == "Wrap":
    ...
    def _draw(self):
        ...
        draw.string("{} test".format(self.test), 0, 6, width=240)
